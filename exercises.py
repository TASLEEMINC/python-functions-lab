...
def apply_discount(product_price, discount_percentage):
    ...
    new_price = product_price - (discount_percentage/100)*product_price
    return new_price
...
#
# Define the function and then call it below.
def sum_to(single_integer_n):
    return sum( range(1, single_integer_n + 1) )
...
def largest(integer_1, integer_2, integer_3):
    if integer_1 > integer_2 and integer_1 > integer_3:
        return integer_1
    
    elif integer_2 > integer_3 and integer_2 > integer_1:
        return integer_2
...
print('Exercise 6: largest number is', largest(110, 64, 52)) 
...
def calculate_tip(bill_amount, tip_percentage):
    amount_of_the_tip = bill_amount * tip_percentage / 100
    return amount_of_the_tip


print('Exercise 7:', calculate_tip(60, 10))      
...
def product(*args):
    result = 1 # initializing result to 1 (since we are multiplying)

    #iterating over the arguments and multiplying them:
    for num in args :
        result =num*result
    return result
...
print('Exercise 9 Result:', basic_calculator(15, 5, "subtract"))